...
def read_candlestick_patterns_from_JSON(configFilePath):
    ...
    for candle_pattern_name in data['patterns']:
        candle_pattern_obj = data['patterns'][candle_pattern_name] # TODO: How to do this assignment inside the FOR loop?

        pattern_candle_count = candle_pattern_obj['candle_count']
        pattern_rules_tree =candle_pattern_obj['rules']

        pattern_rules_string = pattern_config_parser.convertJSONExpressionTreeToString(pattern_rules_tree)
        
        candlestick_patterns[candle_pattern_name] = CandlestickPattern(candle_pattern_name, pattern_candle_count, pattern_rules_string)
...
def load_derived_attributes():
    ...
    for index, row in candlestick_data.iterrows(): 
        ...
        candlestick_dataCopy.at[index, 'is_uptrend']     = True
...
def bootstrap():
    ...
    # Load Candlestick Pattern Definitions
    # ==================================
    bootstrap_success = load_candlestick_patterns() # TODO: there is coupling between property&object identifiers within the JSON and actual DATA
...
if (bootstrap_success):
    ...
    candlestick_dataCopy = candlestick_data.copy() # make a copy for editing data whilst rows are iterated over
    ...
    for index, row in candlestick_data.iterrows():
        ...
        #TODO: make this dynamic based on number of candles needed for pattern
        # dictionary used for dynamic var references
        candlestick_frame = {}
        candlestick_frame['first_candle']   = candlestick_dataCopy.iloc[index - 4,:]
        candlestick_frame['second_candle']  = candlestick_dataCopy.iloc[index - 3,:]
        candlestick_frame['third_candle']   = candlestick_dataCopy.iloc[index - 2,:]
        candlestick_frame['fourth_candle']  = candlestick_dataCopy.iloc[index - 1,:]
        candlestick_frame['fifth_candle']   = candlestick_dataCopy.iloc[index,:]
